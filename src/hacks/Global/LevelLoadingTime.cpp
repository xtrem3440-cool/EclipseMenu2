    ...
    class $hack(LevelLoadingTime) {
        void init() override {
            ...
            tab->addFloatToggle("global.levelloadingtime", 0.1f, 5.0f, "%.2f")
                ->setDescription("Adjusts level loading speed")->handleKeybinds();
        }

        [[nodiscard]] const char* getId() const override { return "Level Loading Time"; }
        [[nodiscard]] int32_t getPriority() const override { return -30; }
    };
